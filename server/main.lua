...
QBCore.Functions.CreateCallback("qb-multicharacter:server:SetupNewCharacter", function(source, cb)
    ...
    MySQL.Async.fetchAll('SELECT * FROM players WHERE license = ?', {license}, function(result)
        for k, v in pairs(result) do
            MySQL.Async.fetchAll('SELECT * FROM playerskins WHERE citizenid = ? AND active = ?', {v.citizenid, 1}, function(skinResult)
                if skinResult and skinResult[1] then
                    plyChars[k] = {skinResult[1].model, skinResult[1].skin, v.cid, json.decode(v.charinfo), v}
                else
                    -- Handle the case where no skins are found for the citizenid
                    plyChars[k] = nil -- or any other value that makes sense for your use case
                end
            end)
        end
        Citizen.Wait(400) -- Changed from Wait() to Citizen.Wait() assuming this is for FiveM
        cb(plyChars)
    end)
end)
